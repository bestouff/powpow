...
# Docker image used for the build — Rust on Debian 12 (bookworm)
...
$(DEB): target/release-bookworm/powpow
	rm -rf _deb
	...
	cp target/release-bookworm/powpow _deb/usr/sbin/powpow
...
target/release-bookworm/powpow: Cargo.toml Cargo.lock $(shell find src -type f) $(shell find migrations -type f)
	mkdir -p target/release-bookworm
	docker run --rm \
		-v "$(CURDIR)":/build \
		-w /build \
		$(RUST_IMAGE) \
		sh -c 'apt-get update && apt-get install -y cmake && cargo build --release && cp target/release/powpow target/release-bookworm/powpow'

clean:
	rm -rf _deb target/release-bookworm $(PKG)_*.deb
