...
function initLoginCheck(prefix) {
    fetch(prefix + '/api/me').then(function(r) { if (r.ok) return r.json(); throw 0; }).then(function(d) {
        var b = document.getElementById('login-btn');
        if (b) {
            b.innerHTML = '<i class="fa-solid fa-user"></i>&nbsp;' + d.first_name + ' ' + d.last_name;
            b.href = prefix + '/person/' + d.id;
            var lo = document.createElement('a'); lo.className = 'navbar-item';
...
function initPersonDetail(prefix) {
    ...
    document.querySelectorAll('.atelier-checkbox').forEach(function(checkbox) {
        checkbox.addEventListener('change', async function() {
            ...
            try {
                var response = await fetch(prefix + '/api/person/' + staffId + '/role', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    var validatedCheckboxes = document.querySelectorAll('.role-validated-checkbox');
    ...
    document.querySelectorAll('.role-chief-checkbox').forEach(function(checkbox) {
        checkbox.addEventListener('change', async function() {
            var atelierId = this.dataset.atelierId;
            var checked = this.checked;
            var validatedCheckbox = document.querySelector('.role-validated-checkbox[data-atelier-id="' + atelierId + '"]');
        ...
        adminCb.addEventListener('change', async function() {
            ...
            try {
                var response = await fetch(prefix + '/api/admin/flags', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ staff_id: staffId, is_admin: adminCb.checked, is_god: godCb ? godCb.checked : false })
                });
        ...
        godCb.addEventListener('change', async function() {
            ...
            try {
                var response = await fetch(prefix + '/api/admin/flags', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ staff_id: staffId, is_admin: adminCb ? adminCb.checked : false, is_god: godCb.checked })
                });
                var data = await response.json();
                if (data.success) {
    ...
    if (saveCommentBtn) {
        saveCommentBtn.addEventListener('click', async function() {
            ...
            try {
                var response = await fetch(prefix + '/api/person/' + staffId + '/comment', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    if (saveContactBtn) {
        saveContactBtn.addEventListener('click', async function() {
            ...
            try {
                var response = await fetch(prefix + '/api/person/' + staffId + '/contact', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    document.querySelectorAll('.pcal-presence-cb').forEach(function(cb) {
        cb.addEventListener('change', async function() {
            ...
            try {
                ...
                var anyChecked = Array.from(cell.querySelectorAll('.pcal-presence-cb')).some(function(c) { return c.checked; });
    ...
    presenceCbs.forEach(function(cb) {
        cb.addEventListener('change', async function() {
            ...
            try {
                ...
                var cell = this.closest('td');
                var anyChecked = Array.from(cell.querySelectorAll('.presence-cb')).some(function(c) { return c.checked; });
                ...
                if (th) {
                    var filledFirst = 0, filledSecond = 0;
                    ...
                    if (countEl) {
                        ...
                        if (spans.length === 2) {
                            ...
                            var firstLabel = spans[0].textContent.replace(/\d+\/\d+/, '').trim();
                            var secondLabel = spans[1].textContent.replace(/\d+\/\d+/, '').trim();
...
function initCalendarEditor(prefix) {
    ...
    document.querySelectorAll('.cal-table td.day-cell').forEach(function(cell) {
        cell.addEventListener('click', function() {
            ...
            fetch(prefix + '/api/calendar/needs-by-day?day=' + day)
                ...
                .catch(function() { showNotification('Erreur chargement', 'danger'); });
        });
    });
    ...
    function initCalendar() {
        ...
        var calContainer = addModal.querySelector('.datetimepicker') || document.querySelector('#calendar-widget').parentElement;
        if (calContainer) {
            var observer = new MutationObserver(function() { highlightDates(); });
    ...
    function highlightDates() {
        ...
        addModal.querySelectorAll('.datepicker-body .datepicker-date').forEach(function(dc) {
            ...
            var dateKey = year + '-' + String(month + 1).padStart(2, '0') + '-' + String(dayNum).padStart(2, '0');
    ...
    var openingDayConfirmText = document.getElementById('opening-day-confirm-text');
...
function initPhotoPage(prefix) {
    ...
    if (createBtn) createBtn.addEventListener('click', function() {
        ...
        .then(function(r) {
            if (r.status === 409) return r.json().then(function(d) { throw new Error(d.error); });
